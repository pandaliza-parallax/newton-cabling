...
#     sudo bash tools/render_batch_eval.sh 0 5      # ep_0000 .. ep_0004 (subset test)
#     sudo bash tools/render_batch_eval.sh 0 100    # the full held-out EVAL set (reads eval_traj_v2/)
set -u
START=${1:-0}
END=${2:-500}
REPO=/home/pandaliza/parallax/newton-cabling
# EVAL variant: read HELD-OUT trajectories (not seated_traj) and write to a separate tree.
TRAJ_DIR=${TRAJ_DIR:-eval_traj_v2}
OUTROOT=${OUTROOT:-$REPO/datagen_v2_eval}
...
for i in $(seq "$START" $((END - 1))); do
    ep=$(printf "ep_%04d" "$i")
    traj=$TRAJ_DIR/$ep/plug_traj.npy
    out=$OUTROOT/$ep
...
echo "[batch] next: Difix the eval renders, then convert -> LeRobot:"
echo "  cd ~/parallax/Difix3D && .venv/bin/python difix_datagen.py \\"
echo "      --src ~/parallax/newton-cabling/datagen_v2_eval --dst ~/parallax/newton-cabling/datagen_v2_difix_eval --target 100"
echo "  cd ~/parallax/openpi && uv run python ~/parallax/newton-cabling/tools/datagen_to_lerobot.py \\"
echo "      --raw ~/parallax/newton-cabling/datagen_v2_difix_eval --repo_id parallax/rj45_sbot_difix_eval"

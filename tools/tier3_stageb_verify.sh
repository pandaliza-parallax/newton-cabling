...
SRC=/home/pandaliza/parallax/data/vla_train/tier3_recovery
GS=/home/pandaliza/parallax/data/vla_train/tier3_recovery_gs
cd "$REPO"
...
for attempt in 1 2 3; do
    ...
    until grep -q "ALL CHUNKS RENDERED" /home/pandaliza/render_tier3.log 2>/dev/null; do sleep 120; done
    ...
    if [ "$N" = "0" ]; then
        ...
        setsid nohup bash "$REPO/tools/tier3_pipeline.sh" \
            > /home/pandaliza/tier3_pipeline.log 2>&1 < /dev/null &
        echo "=== pipeline launched ==="
        exit 0
    fi
    echo "=== $N bad episodes deleted; re-running render driver $(date +%H:%M) ==="
    grep -v "ALL CHUNKS RENDERED" /home/pandaliza/render_tier3.log > /home/pandaliza/render_tier3.log.tmp \
        && mv /home/pandaliza/render_tier3.log.tmp /home/pandaliza/render_tier3.log
    bash tools/render_tier3_campaign.sh >> /home/pandaliza/render_tier3.log 2>&1
done

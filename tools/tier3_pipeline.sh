...
SRC=$ROOT/tier3_recovery            # Stage-A (split) tree: chunk_XX/ep_XXXX + phase.npy
GS=$ROOT/tier3_recovery_gs
FINAL=$ROOT/tier3_recovery_final
DIFIX=$ROOT/tier3_recovery_final_difix
DEP=$ROOT/tier3_recovery_depaused
RENDER_LOG=/home/pandaliza/render_tier3.log
cd "$REPO"
...
python3 - <<'EOF'
import glob, os
root = os.path.expanduser("~/parallax/data/vla_train/tier3_recovery_depaused")
...
cd ~/parallax/openpi
for B in a b c; do
    REPO_ID=Parallax-Worlds/cable_tier3rec_${B}_depaused
...
echo "=== TIER3 PIPELINE DONE + PUSHED $(date +%H:%M) ==="

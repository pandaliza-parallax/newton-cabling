...
COND=/home/pandaliza/parallax/data/vla_train/evalmatrix_conditions.json
...
echo "=== waiting for the tier-2 data push (data-first ordering) $(date +%H:%M) ==="
# NOTE: the verify gate's GAVE UP is deliberately ignored here — its 2026-08-21
# firing was a glob false-positive (chunk_XX_raw dirs); data was re-verified clean
# by hand and the pipeline launched manually. Gate on the pipeline log only.
until grep -q "TIER2 PIPELINE DONE" /home/pandaliza/tier2_pipeline.log 2>/dev/null; do sleep 120; done
...
( cd /home/pandaliza/parallax/openpi && setsid nohup uv run python $REPO/tools/serve_act_policy.py \
    --ckpt /home/pandaliza/parallax/openpi/checkpoints/act_c10_depaused --port 8000 \
...
run_block act_c10_depaused

echo "=== pi05 block: pausing batch difix $(date +%H:%M) ==="
pkill -f "inc_difix_loo[p]"; pkill -f "difix_datage[n]"; sleep 5
pkill -f "serve_act_polic[y]"; sleep 3
...
sleep 5
run_block pi05_21000

echo "=== cleanup: resume batch difix, stop policy server $(date +%H:%M) ==="
pkill -f "serve_polic[y]"; sleep 3
setsid nohup bash $REPO/tools/inc_difix_loop.sh >> /home/pandaliza/difix_incremental.log 2>&1 < /dev/null &
python3 - "$OUTROOT" <<'EOF'
...
seat = {m: sum(1 for n, s, _ in rows if n.startswith(m) and s) for m in ("act_c10_depaused", "pi05_21000")}
print("SEAT RATES:", seat, "of", len(rows) // 2, "each")
EOF

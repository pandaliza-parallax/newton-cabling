...
#            --policy.dir=checkpoints/pi05_rj45_sbot_difix/10000
...
OUTROOT=${OUTROOT:-$REPO/evalrun_v2}   # override to keep runs side by side (each ep dir is rm -rf'd)
...
for i in $(seq "$START" $((END - 1))); do
    ...
    .venv/bin/python record_sbot_scene_gs.py \
        ...
        --policy-server "$SERVER" --eval-max-steps 300 --eval-grasped-start ${DIFIX_FLAG:-} \
        --out "$out" > "$out.log" 2>&1
    ran=$((ran + 1))

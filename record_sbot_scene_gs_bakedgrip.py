"""BAKED-GRIP variant of record_sbot_scene_gs.py (copy). The gripper is rendered as part of the
ORIGINAL wrist_3 splat (flat/wrist_3_link.ply, gripper baked in) instead of separate articulating
finger splats: LINK_SPLATS is the 7 arm links only, --wrist3-ply defaults to None (uses flat/), and
there are no cut fingers / no jaw open-close in the render. Everything else matches the original.
...
LINK_SPLATS: tuple[str, ...] = (   # BAKED-GRIP variant: 7 arm links only. The gripper is BAKED into
    "base_link", "shoulder_link", "upper_arm_link", "forearm_link",   # the wrist_3 splat (flat/), so
    "wrist_1_link", "wrist_2_link", "wrist_3_link",                   # NO separate articulating fingers.
)
...
def main() -> None:
    ...
    ap.add_argument("--wrist3-ply", default=None,
                    help="BAKED-GRIP variant: default None -> wrist_3 loads the ORIGINAL flat/wrist_3_link.ply "
                         "(gripper baked in). There are no separate finger splats in this script, so no crop "
                         "is needed. (Pass a path to override.)")
    ...
    if args.wrist_cam and args.wrist_cam_from_usd:
    ...
    print(
        ...
        f"+ {len(LINK_SPLATS)} robot links (7 arm; gripper baked into wrist_3)"
    )
